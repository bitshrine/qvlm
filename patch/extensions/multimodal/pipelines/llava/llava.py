...
def expand2square(pil_img: Image.Image, background_color: Tuple[int]) -> Image.Image:
    ...
    if (pil_img.mode == 'L'):
        try:
            background_color = int(sum(background_color) / len(background_color))
        except:
...
class LLaVA_v1_5_13B_Pipeline(LLaVA_v0_13B_Pipeline):
    ...
    def embed_images(self, images: List[Image.Image]) -> torch.Tensor:
        ...
        image_mean = self.image_processor.image_mean if self.image_processor.image_mean != None else [0.48145466, 0.4578275, 0.40821073]
        images = [
