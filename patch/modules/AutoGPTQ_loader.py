...
def load_quantized(model_name):
    ...
    if shared.args.checkpoint:
        ...
        for ext in ['.safetensors', '.pt', '.bin']:
            ...
            if len(found) > 0:
                if len(found) > 1:
                    #path_to_model = Path(shared.args.model_dir)
                    logger.info(f'Several {ext} shards have been found. Attempting to load them with AutoGPTQForCausalLM.from_quantized({path_to_model})')
                    shards = len(found)
                pt_path = found[-1]
                break
    ...
    if (shards > 1):
        del params['model_basename']
        params.update({"model_path_or_name": path_to_model})
